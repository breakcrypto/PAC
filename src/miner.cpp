...
// ***TODO*** ScanHash is not yet used in Paccoin
//
...
{
    LogPrintf("PaccoinMiner -- started\n");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("PaccoinMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("PaccoinMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
            ...
            while (true)
            {
                ...
                while (true)
                {
                    hash = pblock->GetPoWHash(pindexPrev->nHeight + 1);
    ...
    {
        LogPrintf("PaccoinMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("PaccoinMiner -- runtime error: %s\n", e.what());
        return;
    }
}
